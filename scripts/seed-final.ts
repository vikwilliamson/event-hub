...
async function seedEvents() {
  ...
  try {
    // First create organizer documents
    ...
    for (const eventData of sampleEvents) {
      ...
      const eventDoc = {
        ...
        // Scheduling
        startsAt: Timestamp.fromDate(eventData.startsAt),
        endsAt: eventData.endsAt ? Timestamp.fromDate(eventData.endsAt) : null,
        ...
        updatedAt: Timestamp.fromDate(now),
        cancelledAt: null,
        publishedAt: eventData.status === 'published' ? Timestamp.fromDate(now) : null,
      };
