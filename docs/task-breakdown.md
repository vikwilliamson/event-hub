...
- 2026-08-11 — **Vercel-compatible data store (Vercel KV / Upstash Redis).** The local JSON file store can't run on Vercel's serverless filesystem (read-only, ephemeral, per-instance → empty listing + erroring writes). Added `KvStore` behind the existing `Store` interface (`src/lib/store/index.ts`): the whole `StoreData` blob under one KV key, read-modify-write with no in-memory cache so writes are visible across instances. `getStore()` now auto-selects KV when `KV_REST_API_URL`/`KV_REST_API_TOKEN` (or the `UPSTASH_*` pair) are set, else the JSON file for local/dev/E2E. Seeding is unchanged (`npm run seed` upserts via `getStore()`, so it seeds KV when those env vars are present). 4 new `KvStore` unit tests (fake KV client). Deploy runbook in `docs/deploy-vercel.md`; `.env.example` updated. 106 unit tests, 10/10 E2E, tsc, lint, build green. (Deploy itself is user-run — needs interactive `vercel login`.)
